...
from app.handler.pgp_strategies import (
    GnuPGEncryptionStrategy,
    PgpEncryptionStrategyCoordinator,
    PgpyEncryptionStrategy,
    PublicKeyValidationStrategy,
)
from app.handler.spamd_result import (
    SpamdResult,
    SPFCheckResult,
)
from app.handler.steps.alias_resolution import (
    AliasResolutionStep,
    ForwardAliasResolutionStrategy,
    ReplyAliasResolutionStrategy,
)
from app.handler.steps.contact_management import (
    ContactManagementStep,
    ForwardContactManagementStrategy,
    ReplyContactManagementStrategy,
)
from app.handler.steps.delivery import (
    DeliveryStep,
    ForwardDeliveryStrategy,
    ForwardMailboxDeliveryStrategy,
    ReplyDeliveryStrategy,
)
from app.handler.steps.header_rewrite import (
    ForwardPostHeaderRewriteStrategy,
    ForwardPreHeaderRewriteStrategy,
    PostHeaderRewriteStep,
    PreHeaderRewriteStep,
    ReplyPostHeaderRewriteStrategy,
    ReplyPreHeaderRewriteStrategy,
)
from app.handler.steps.pgp_encryption import (
    ForwardPgpEncryptionStrategy,
    PgpEncryptionStep,
    ReplyPgpEncryptionStrategy,
)
from app.handler.steps.security_check import (
    ForwardMailboxSecurityCheckStrategy,
    ForwardSecurityCheckStrategy,
    ReplySecurityCheckStrategy,
    SecurityCheckStep,
)
...
def prepare_pgp_message(
    ...
    encrypted_data = PGP_ENCRYPTION_STRATEGY.encrypt(
        msg_bytes,
        pgp_fingerprint,
        public_key,
        ctx,
...
PGP_ENCRYPTION_STRATEGY = PgpEncryptionStrategyCoordinator(
    primary=GnuPGEncryptionStrategy(),
    validator=PublicKeyValidationStrategy(),
    fallback=PgpyEncryptionStrategy(),
)


def send_invalid_mailbox_notification(
    user: User,
    mailbox: Mailbox,
    alias: Alias,
    subject: str,
...
def run_forward_mailbox_pipeline(context: EmailProcessingContext) -> Tuple[bool, str]:
    result = EmailProcessingPipeline(
        [
            SecurityCheckStep(
                ForwardMailboxSecurityCheckStrategy(
                    quarantine_disabled_mailbox_email=quarantine_disabled_mailbox_email,
                    handle_spam=handle_spam,
                    send_invalid_mailbox_notification=send_invalid_mailbox_notification,
                )
            ),
            PreHeaderRewriteStep(ForwardPreHeaderRewriteStrategy()),
            PgpEncryptionStep(
                ForwardPgpEncryptionStrategy(prepare_pgp_message=prepare_pgp_message)
            ),
            PostHeaderRewriteStep(
                ForwardPostHeaderRewriteStrategy(
                    replace_sl_message_id_by_original_message_id=replace_sl_message_id_by_original_message_id,
                    replace_header_when_forward=replace_header_when_forward,
                    add_alias_to_header_if_needed=add_alias_to_header_if_needed,
                )
            ),
            DeliveryStep(ForwardMailboxDeliveryStrategy()),
        ]
    ).run(context)
    return result


def build_forward_pipeline() -> EmailProcessingPipeline:
    return EmailProcessingPipeline(
        [
            AliasResolutionStep(
                ForwardAliasResolutionStrategy(
                    try_auto_create=try_auto_create,
                    handle_email_sent_to_ourself=handle_email_sent_to_ourself,
                )
            ),
            ContactManagementStep(
                ForwardContactManagementStrategy(
                    get_or_create_contact=get_or_create_contact,
                    get_or_create_reply_to_contact=get_or_create_reply_to_contact,
                )
            ),
            SecurityCheckStep(ForwardSecurityCheckStrategy()),
            DeliveryStep(
                ForwardDeliveryStrategy(
                    mailbox_pipeline_runner=run_forward_mailbox_pipeline,
                    send_invalid_mailbox_notification=send_invalid_mailbox_notification,
                    copy_message=copy,
                )
            ),
...
def build_reply_pipeline() -> EmailProcessingPipeline:
    return EmailProcessingPipeline(
        [
            AliasResolutionStep(ReplyAliasResolutionStrategy()),
            ContactManagementStep(ReplyContactManagementStrategy()),
            SecurityCheckStep(
                ReplySecurityCheckStrategy(
                    handle_unknown_mailbox=handle_unknown_mailbox,
                    handle_spam=handle_spam,
                )
            ),
            PreHeaderRewriteStep(ReplyPreHeaderRewriteStrategy()),
            PgpEncryptionStep(
                ReplyPgpEncryptionStrategy(prepare_pgp_message=prepare_pgp_message)
            ),
            PostHeaderRewriteStep(
                ReplyPostHeaderRewriteStrategy(
                    get_alias_recipient_name=get_alias_recipient_name,
                    replace_header_when_reply=replace_header_when_reply,
                    replace_original_message_id=replace_original_message_id,
...
def handle_forward(envelope, msg: Message, rcpt_to: str) -> List[Tuple[bool, str]]:
    ...
    """
    result = build_forward_pipeline().run(
        EmailProcessingContext(envelope=envelope, msg=msg, rcpt_to=rcpt_to)
    )
    return result
...
def forward_email_to_mailbox(
    ...
    return run_forward_mailbox_pipeline(
        EmailProcessingContext(
            ...
            user=user,
            contact=contact,
            reply_to_contacts=list(reply_to_contacts),
            mailbox=mailbox,
...
def handle_reply(
    ...
    """
    result = build_reply_pipeline().run(
        EmailProcessingContext(
            envelope=envelope,
            msg=msg,
            rcpt_to=rcpt_to,
