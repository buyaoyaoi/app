...
def handle_forward(envelope, msg: Message, rcpt_to: str) -> List[Tuple[bool, str]]:
    ...
    """
    from app.handler.steps.protocol import EmailProcessingPipeline
    ...
    from app.handler.steps.security_check import SecurityCheckStep

    ctx = EmailProcessingContext(
        msg=msg,
        envelope=envelope,
        rcpt_to=rcpt_to,
        phase="forward",
    ...
    pipeline.add_step(AliasResolutionStep())
    pipeline.add_step(ContactManagementStep())
    pipeline.add_step(SecurityCheckStep())

    early_return = pipeline.execute(ctx)
        ...
        if not mailbox.verified:
            ...
            ret.append(
                forward_email_to_mailbox(
                    alias, copy(msg), contact, envelope, mailbox, user, reply_to_contacts
                )
            )
...
def forward_email_to_mailbox(
    ...
    from app.handler.steps.pgp_step import PGPEncryptionStep
    from app.handler.steps.delivery import DeliveryStep

    per_mailbox_ctx = EmailProcessingContext(
        msg=msg,
        envelope=envelope,
        ...
        contact=contact,
        mailbox=mailbox,
        reply_to_contacts=reply_to_contacts,
        email_log=email_log,
        phase="forward_per_mailbox",
    )

    per_mailbox_pipeline = EmailProcessingPipeline()
    per_mailbox_pipeline.add_step(HeaderRewriteStep())
    per_mailbox_pipeline.add_step(PGPEncryptionStep())
    per_mailbox_pipeline.add_step(DeliveryStep())

    early_return = per_mailbox_pipeline.execute(per_mailbox_ctx)
    if early_return is not None:
        return early_return
...
def handle_reply(
    ...
    pipeline.add_step(ContactManagementStep())
    pipeline.add_step(SecurityCheckStep())
    pipeline.add_step(HeaderRewriteStep())
    pipeline.add_step(PGPEncryptionStep())
    pipeline.add_step(DeliveryStep())
