...
from app.handler.pipeline import EmailProcessingPipeline
...
def handle_forward(envelope, msg: Message, rcpt_to: str) -> List[Tuple[bool, str]]:
    ...
    """
    pipeline = EmailProcessingPipeline.for_forward(envelope, msg, rcpt_to)

    initial_results = pipeline.execute()
    if initial_results:
        return initial_results

    mailbox_results = pipeline.execute_for_each_mailbox()
    return mailbox_results
...
def handle_reply(
    ...
    """
    pipeline = EmailProcessingPipeline.for_reply(envelope, msg, rcpt_to, notified_mailboxes)

    results = pipeline.execute()
    if results:
        is_success, smtp_status = results[0]
        return is_success, smtp_status
